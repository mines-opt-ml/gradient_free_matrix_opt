...
Runs every recovery method on every objective at its tuned configuration, and
records the full trace of each run. Completed runs already present in the CSV
are skipped, so an interrupted invocation can be resumed by rerunning it.
...
def completed_runs() -> set[tuple[str, str, str]]:
    ...
    finishes, so one success row is enough to mark the pair as done. Including
    the configuration means results carried over from a different query budget
    are rerun rather than silently accepted.
    """
    ...
    with QUERY_RESULTS_FILE.open(newline="", encoding="utf-8") as file:
        for row in csv.DictReader(file):
            if row["status"] == "success":
                completed.add((row["algorithm"], row["objective"], row["parameter_configuration"]))
    return completed
...
def run_query_experiment(tuning_results: dict[str, Any]) -> None:
    ...
    for method in MATRIX_RECOVERY_REGISTRY:
        for objective_name in OBJECTIVE_NAMES:
            optimizer, configuration_json = build_optimizer(tuning_results, method, objective_name)
            if (method, objective_name, configuration_json) in completed:
                print(f"Skipping completed query run: {method} / {objective_name}")
                continue

            print(f"Query run: {method} / {objective_name}", flush=True)
            sample_count: int | str = "" if method == RANGEFINDER else optimizer.samps_per_iter
